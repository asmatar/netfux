    ...
    <div className="fixed top-0 left-0 w-full h-full right-0 bottom-0 outline-none overflow-x-hidden overflow-y-auto bg-black/60 z-50 flex items-center justify-center animate-fade">
        <div className="bg-[#181818] relative">
            ...
            <div className="flex h-full">
                ...
                <div classNameName="w-[375px]">
                    <div className="p-4">
                        <span className="px-2 py-1 leading-none bg-orange-200 text-red-600 rounded-full font-semibold uppercase tracking-wide text-xs">{casting?.known_for_department}</span>
                        <h2 className="mt-2 mb-2 font-bold">
                            <span classNameName="text-xs mr-2 font-bold">{casting?.gender === 2 ? "Mr" : "Mrs"}</span>{casting?.name}
                        </h2>
                        <p className="text-sm text-justify">{casting?.biography.substring(0, 600) + " ..."}</p>
                    </div>
                    <div className="p-4 border-t border-b border-red-600 text-xs text-gray-700">
                    ...
                    <div className="p-4 flex items-center text-sm text-white">
                        <span className="ml-2 flex gap-x-2 items-center">popularity: {casting?.popularity}
