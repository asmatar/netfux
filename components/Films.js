...
const Films = ({films}) => {
 ...
 const sameMovie = favorite.find(item => item.id === films.id)
  ...
  return (
    <>
      <Link href={`/details/${films.id}`} className="flex flex-col gap-y-2 hover:scale-105 transition-all duration-200 ease relative" >
      <div className="relative h-[14vw] w-[full] overflow-hidden rounded text-red-700">
          { sameMovie !== undefined ? <button className="rounded-full border-red-700/60 p-1 sm:p-1 border-2 hover:border-red-700 transition duration-200 ease-out absolute top-2 right-4 z-50 rotate-180" onClick={() => handleRemoveFromFavorite(films.id)}>
      ...
      {/* <p className="text-center text-sm">{films.title}</p> */}
      </Link>
    </>
  )
}
